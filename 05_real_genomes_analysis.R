...
cp_diagnostics <- function(x, s, J_true, alpha = 0.05) {
  if (x <= 0 || x >= s) {
    if (x <= 0) {
      J_lo <- 0
      J_hi <- qbeta(1 - alpha / 2, 1, s)
    } else {
      J_lo <- qbeta(alpha / 2, s, 1)
      J_hi <- 1
    }
    return(tibble(
      x = x, J_hat = x / s, J_lo = J_lo, J_hi = J_hi,
  ...
  tibble(
    x = x, J_hat = J_hat, J_lo = J_lo, J_hi = J_hi, rho = rho,
...
process_pair_fast <- function(pair_id, accA, accB, f_min, k) {
  ...
  run_at_s <- function(s_target, label) {
    ...
    tibble(
      s_label = label, s_target = s_target, s_used = cmp$s_used,
  ...
  bind_rows(run_at_s(1000, "default_s1000"), run_at_s(s_star, "formula_s_star")) |>
    mutate(
      pair_id = pair_id, accA = accA, accB = accB,
...
results_pairs <- pmap_dfr(
  list(
    fastani$pair_id, fastani$accession_A, fastani$accession_B,
...
if (file.exists(selection_path)) {
  ...
  rank_bias_by_anchor <- pair_level_ranked |>
    ...
    summarise(
      n_pairs = n(), median_bias_pct = median(J_bias_pct),
      max_bias_pct = max(J_bias_pct), .groups = "drop"
    ) |>
